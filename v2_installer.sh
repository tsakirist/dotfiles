...
function _checkfile() {
	if [ ! -f $1 ]; then
		echo "${bold}${red}ERROR${reset}: Can't find ${1} in this directory. Installer needs all the contents of the github repository."
...
function _sublimekeybindings() {
	_checkfile sublime_keybindings.json
	echo "Setting ${bold}${red}Sublime keybindings${reset}..."
	mv -v sublime_keybindings.json "$HOME/.config/sublime-text-3/Packages/User/Default (Linux).sublime-keymap"
}
...
function _powerlineconfig() {
	...
	mv -v powerline_configs/themes/shell/default.json "$HOME/.local/lib/python2.7/site-packages/powerline/config_files/themes/shell" -v --backup=numbered

	echo "Setting ${bold}${red}colorschemes/default.json${reset}..."
	mv -v powerline_configs/colorschemes/default.json "${HOME}/.local/lib/python2.7/site-packages/powerline/config_files/colorschemes" -v --backup=numbered
}

function _dconfsettings() {
	_checkfile dconf/settings.dconf
	echo "Setting ${bold}${red}dconf settings${reset}..."
